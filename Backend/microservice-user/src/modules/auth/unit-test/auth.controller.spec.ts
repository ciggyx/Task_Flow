...
describe('AuthController', () => {
  ...
  describe('restorePassword', () => {
    ...
    it('should handle errors thrown by authService.restorePassword', async () => {
      ...
      mockAuthService.restorePassword.mockRejectedValue(
        new Error('Restore failed'),
      );

      await expect(controller.restorePassword(restoreDto)).rejects.toThrow(
        'Restore failed',
      );
    });
  });
  ...
  describe('validatePermission', () => {
    it('should call authService.validateTokenAndPermissions and return true', async () => {
      ...
      const result = await controller.validatePermission(
        authHeader,
        requiredPermissions,
      );
