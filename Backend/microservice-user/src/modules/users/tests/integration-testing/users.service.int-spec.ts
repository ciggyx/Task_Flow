...
describe('UsersService (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    permissionRepo = moduleRef.get<Repository<Permission>>(
  ...
  describe('getIdbyEmail', () => {
    ...
    it('lanza NotFound si no existe', async () => {
      await expect(service.getIdbyEmail('nope@test.com')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('findByEmail', () => {
    it('recupera usuario con roles y permisos', async () => {
      ...
      const found = await service.findByEmail('carl@test.com', [
        'roles',
        'roles.permissions',
      ]);
      expect(found).toBeDefined();
  ...
  describe('findOneByEmailWithRolesAndPermissions', () => {
    it('devuelve usuario con roles y permisos', async () => {
      ...
      const found =
        await service.findOneByEmailWithRolesAndPermissions('f@test.com');
      expect(found).toBeDefined();
      expect(found.roles).toHaveLength(1);
      expect(found.roles[0].permissions).toHaveLength(1);
    });
    ...
    it('lanza NotFoundException si no existe', async () => {
      await expect(
        service.findOneByEmailWithRolesAndPermissions('noone@test.com'),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('updatePassword', () => {
    ...
    it('lanza BadRequestException si la nueva contraseña es inválida', async () => {
      ...
      await expect(service.updatePassword(u.id, '')).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.updatePassword(u.id, '123')).rejects.toThrow(
        BadRequestException,
      );
    });

    it('lanza NotFoundException si el usuario no existe', async () => {
      await expect(service.updatePassword(9999, 'anotherpass')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
