...
describe('RolesService', () => {
  ...
  describe('create', () => {
    it('should create and save a role with permissions', async () => {
      ...
      const permissions: Permission[] = [{ id: 1, name: 'permiso', description: 'desc', roles: [] }];
  ...
  describe('findAll', () => {
    it('should return all roles', async () => {
      const roles: Role[] = [
        { id: 1, code: 'R1', name: 'Rol1', description: 'desc', permissions: [], users: [] },
      ];
  ...
  describe('findOne', () => {
    it('should return a role if found', async () => {
      const role: Role = { id: 1, code: 'R1', name: 'Rol1', description: 'desc', permissions: [], users: [] };
  ...
  describe('update', () => {
    it('should update a role', async () => {
      ...
      const permissions: Permission[] = [{ id: 2, name: 'permiso2', description: 'desc2', roles: [] }];
      const existing: Role = { id: 1, code: 'R1', name: 'Old', description: 'Old desc', permissions: permissions, users: [] };
    ...
    it('should throw error if code, name, description and permissions are missing', async () => {
      await expect(service.update(1, { code: '', name: '', description: '', permissions: [] })).rejects.toThrow(
    ...
    it('should throw NotFoundException if role does not exist', async () => {
      mockRolesRepo.findOne.mockResolvedValue(null);
      await expect(service.update(1, { name: 'x', description: 'y' })).rejects.toThrow(NotFoundException);
    });
  });
