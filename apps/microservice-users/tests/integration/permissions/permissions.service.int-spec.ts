...
describe('PermissionsService (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    repo = moduleRef.get<Repository<Permission>>(getRepositoryToken(Permission));
  });
  ...
  describe('create', () => {
    ...
    it('create: falla si el permiso ya existe', async () => {
      ...
      await expect(service.create(dto as Permission)).rejects.toThrow(ConflictException);
    });

    it('create: falla si faltan campos', async () => {
      await expect(service.create({ name: '' } as Permission)).rejects.toThrow();
    });
  });
