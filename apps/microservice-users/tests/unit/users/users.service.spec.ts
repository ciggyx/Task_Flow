...
import { Role } from '../../../src/modules/roles/entities/role.entity'; // Asumiendo que existe una entidad Role
...
describe('UsersService', () => {
  ...
  describe('getIdbyEmail', () => {
    ...
    it('should throw NotFoundException if user is not found', async () => {
      ...
      await expect(service.getIdbyEmail(email)).rejects.toThrow(NotFoundException);
      await expect(service.getIdbyEmail(email)).rejects.toThrow('No matching user found');
    });
  });
  describe('findByName', () => {
  ...
  describe('updateRol', () => {
    ...
    const foundRoles = [{ id: 1, code: 'ADMIN' } as Role, { id: 2, code: 'USER' } as Role];
    ...
    it('should throw NotFoundException if user is not found', async () => {
      ...
      await expect(service.updateRol(userId, updateDto)).rejects.toThrow(NotFoundException);
    ...
    it('should throw NotFoundException if no matching roles are found', async () => {
      ...
      await expect(service.updateRol(userId, updateDto)).rejects.toThrow(NotFoundException);
      await expect(service.updateRol(userId, updateDto)).rejects.toThrow('No matching role found');
  ...
  describe('updatePassword', () => {
    ...
    it('should throw BadRequestException if password is invalid', async () => {
      ...
      await expect(service.updatePassword(userId, 'short')).rejects.toThrow(BadRequestException);
    ...
    it('should throw BadRequestException if password is empty', async () => {
      ...
      await expect(service.updatePassword(userId, '')).rejects.toThrow(BadRequestException);
      expect(bcrypt.hash).not.toHaveBeenCalled(); // No debe intentar hashear
    });
    it('should throw NotFoundException if user is not found', async () => {
      mockUserRepository.findOneBy.mockResolvedValue(null);
      ...
      await expect(service.updatePassword(userId, newPassword)).rejects.toThrow('User not found');
      expect(bcrypt.hash).not.toHaveBeenCalled(); // No debe intentar hashear
    });
    it('should throw error if userRepo.update fails', async () => {
      mockUserRepository.findOneBy.mockResolvedValue(userFound);
      mockUserRepository.update.mockRejectedValue(new Error('Could not update password'));
