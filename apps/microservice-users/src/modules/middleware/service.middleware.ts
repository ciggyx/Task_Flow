...
export class AuthService {
  ...
  async validateTokenAndPermissions(
    ...
    if (!user.roles || user.roles.length === 0) {
      throw new UnauthorizedException('Roles o permisos no definidos para el usuario.');
    }

    const permissionsStrings = user.roles.flatMap((role) => role.permissions.map((p) => p.name));
